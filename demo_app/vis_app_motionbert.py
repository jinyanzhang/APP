...
def get_pose3D(video_path, output_dir):
    ...
    model = MotionBERT(maxlen=243, dim_feat=512, mlp_ratio=2, depth=5, dim_rep=512, num_heads=8, att_fuse=True, return_mid=True)
    
    checkpoint = torch.load('/data-home/checkpoints/motionbert-finetune-h36m-author.pth.tr', map_location=lambda storage, loc: storage)
    state_dict = {}
    ...
    app = AdaptivePosePoolingv2(
        ...
        dim_img=128, dim_pose=512, dim_out=3, drop=.0, mlp_ratio=4., act_layer=nn.GELU,
        backbone_features=[32, 64, 128, 256])
    
    checkpoint = torch.load('/data-home/checkpoints/appv2-motionbert-finetune-h36m-sh.pth.tr', map_location=lambda storage, loc: storage)
    state_dict = {}
    for key in checkpoint['model'].keys():
        state_dict[key.replace('module.', '')] = checkpoint['model'][key].clone()
...
if __name__ == "__main__":
    ...
    output_dir = f'/data-home/output/{video_name}_app_motionbert/'
