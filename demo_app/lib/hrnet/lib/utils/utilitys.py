...
from lib.hrnet.lib.utils.transforms import crop_image
...
# for app
def get_center_scale(box):
    center = np.array([(box[0] + box[2]) / 2, (box[1] + box[3]) / 2], dtype=np.float32)
    wh = np.array([box[2] - box[0], box[3] - box[1]], dtype=np.float32)
    scale = wh / 200
    return center, scale
...
def PreProcess(image, bboxs, cfg, num_pos=2):
    ...
    for bbox in bboxs[:num_pos]:
        h = data_numpy.shape[-2]
        
        c, s = get_center_scale(bbox)

        r = 0

        input, trans, inv_trans = crop_image(data_numpy, c, s, (192, 256))
        crops.append(input.copy())
    ...
    return inputs, crops, data_numpy, trans, inv_trans
