...
class MotionAGFormer(nn.Module):
    ...
    def __init__(self, n_layers, dim_in, dim_feat, dim_rep=512, dim_out=3, mlp_ratio=4, act_layer=nn.GELU, attn_drop=0.,
                 ...
                 use_temporal_similarity=True, temporal_connection_len=1, use_tcn=False, graph_only=False, return_mid=False,
        ...
        # Take MotionAGFormer-large as example
        # nn.Linear(3, 128)
        ...
        # nn.Sequential(nn.Linear(128, 512), nn.Tanh)
    ...
    def forward(self, x, return_rep=False):
        ...
        if self.return_mid:
            return x
        x = self.rep_logit(x)#bs,T,17,512
        if return_rep:
            return x

        x = self.head(x)#bs,T,17,3
...
def _test():
    ...
    model = MotionAGFormer(n_layers=16, dim_in=3, dim_feat=128, mlp_ratio=4, hierarchical=False, num_heads=8,
                           use_tcn=False, graph_only=False, n_frames=t, return_mid=True).cuda()
    model.eval()
