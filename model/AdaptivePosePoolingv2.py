...
class AdaptivePosePoolingv2(nn.Module):
    def __init__(self, num_joints=17, n_frames=243, n_layers=6, n_heads=8, kernel_size=3, alpha=0.9, drop_path=0.2,
...
def _test():
    ...
    b, c, t, j = 1, 3, 243, 17
    poses = torch.randn((b, t, j, f_p)).cuda()
    f_maps = [
        torch.randn((1, 1, 32, 64, 48)).cuda(),
        torch.randn((1, 1, 64, 32, 24)).cuda(),
        torch.randn((1, 1, 128, 16, 12)).cuda(),
        torch.randn((1, 1, 256, 8, 6)).cuda(),
        ]
    poses_crop = torch.randint(low=0, high=256, size=(b, t, j, c - 1)).float().cuda()

    model = AdaptivePosePoolingv2(dim_pose=f_p).cuda()
    model.eval()
