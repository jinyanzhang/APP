...
class STCFormer(nn.Module):
    def __init__(self, dim_in=2, layers=6, d_hid=512, n_frames=243, num_joints=17, return_mid=False):
        ...
        self.pose_emb = nn.Linear(dim_in, d_hid, bias=False)
        self.gelu = nn.GELU()
        self.stcformer = MetaFormer(layers, n_frames, num_joints, d_hid)
...
def _test():
    ...
    model = STCFormer(return_mid=True).cuda()
