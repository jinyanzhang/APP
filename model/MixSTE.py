...
from model.modules.rela import RectifiedLinearAttention
...
class MixSTE(nn.Module):
    def __init__(self, n_frames=9, num_joints=17, in_chans=2, embed_dim_ratio=32, depth=4,
                 num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.2,  norm_layer=None, return_mid=False):
        ...
        self.Temporal_pos_embed = nn.Parameter(torch.zeros(1, n_frames, embed_dim))
...
def _test():
    ...
    model = MixSTE(in_chans=2, embed_dim_ratio=512, depth=8, num_heads=8, mlp_ratio=2.,
                   qkv_bias=True, qk_scale=None, drop_path_rate=0.1, n_frames=243, num_joints=17, return_mid=True).cuda()
    
    # checkpoint = torch.load('/data-home/checkpoints/mixste-h36m-cpn-author.pth.tr')
    # state_dict = {}
    ...
    # torch.save(new_checkpoint, 'mixste-h36m-cpn-author.pth.tr')
