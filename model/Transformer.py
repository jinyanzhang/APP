...
class Transformer(nn.Module):
    def __init__(self, dim_in=3, dim_out=3, dim_feat=128, dim_rep=512, depth=16, num_heads=8, mlp_ratio=4, 
                 num_joints=17, n_frames=243, qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, att_fuse=True, mode='stage_st_st',
                 return_mid=False):
        super().__init__()
        ...
        self.blocks_st = nn.ModuleList([
            Block(
                ...
                st_mode='stage_' + mode.split('_')[1])
            for i in range(depth)])
        self.blocks_ts = nn.ModuleList([
            Block(
                dim=dim_feat, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, 
                st_mode='stage_' + mode.split('_')[2])
            for i in range(depth)])
...
def _test():
    ...
    model = Transformer(dim_in=3, dim_out=3, dim_feat=128, dim_rep=512, depth=12, num_heads=8,
                        ...
                        drop_path_rate=0., att_fuse=True, mode='stage_ts_ts').cuda()
    model.eval()
