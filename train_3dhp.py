...
from data.reader.motion_dataset_3dhp import MotionDataset3DHP, flip_data
...
def evaluate(args, model, test_loader, datareader, device, rank=0, dist_size=None, master=True):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            poses, target, index, source = batch
...
def train(args, opts):
    ...
    train_dataset = MotionDataset3DHP(args, args.subset_list, 'train')
    test_dataset = MotionDataset3DHP(args, args.subset_list, 'test', rank, world_size)
