...
from data.reader.motion_dataset_3dhp import MotionFeatureDataset3DHP, flip_data
...
def evaluate(args, model, test_loader, datareader, device, rank=0, dist_size=None, master=True):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            *feature_maps, poses, poses_crop, target, index, source = batch
            
            feature_maps = [f.to(device) for f in feature_maps]
            target = target[:, p_center:p_center+1]
            
            poses_crop = poses_crop.to(device)
            poses, target = poses.to(device), target.to(device)
            index, source = index.to(device), source.to(device)

            predicted_3d_pos = model(feature_maps, poses, poses_crop)
    ...
    # np.savez('a.npz', preds=preds, targets=targets)
...
def train(args, opts):
    ...
    train_dataset = MotionFeatureDataset3DHP(args, args.subset_list, 'train')
    test_dataset = MotionFeatureDataset3DHP(args, args.subset_list, 'test', rank, world_size)
