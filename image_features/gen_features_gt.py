...
class GenSequenceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # read images
        images = np.zeros((len(image_paths), 3, 384, 288), dtype=np.float32)
        f_images = np.zeros((len(image_paths), 3, 384, 288), dtype=np.float32)
...
if __name__ == '__main__':
    ...
    if detector_type == 'hrnet':
        ...
        model = cpn.build_model('CPN50_256x192')
        model = hrnet.load_model(model, f'{pretrained_dir}/coco/CPN_256x192.pth.tar')
    elif detector_type == 'gt':
        model = hrnet.build_model('hrnet_w48_384_288')
        model = hrnet.load_model(model, f'{pretrained_dir}/h36m/final_state.pth')
    else:
        raise ValueError
    model = model.cuda()
    model.eval()
