...
def loss_norml2(predicted):
    return torch.mean(torch.norm(predicted, dim=predicted.ndim - 1))


def loss_model(predicted, target, pretrained=True):
    total_loss = loss_mpjpe(predicted[1], target)
    total_loss += loss_norml2(predicted[1])
    if not pretrained:
        total_loss += loss_mpjpe(predicted[0], target)
        total_loss += loss_norml2(predicted[0])
    return total_loss
