...
from loss.pose3d import loss_mpjpe, n_mpjpe, loss_velocity, loss_limb_var
from loss.pose3d import loss_limb_gt, loss_angle, loss_angle_velocity
...
from data.const import H36M_JOINT_TO_LABEL, H36M_UPPER_BODY_JOINTS, H36M_LOWER_BODY_JOINTS
from data.const import H36M_1_DF, H36M_2_DF, H36M_3_DF
from data.reader.h36m import MultipleDataReaderH36M
from data.reader.motion_dataset import MultipleMotionFeatureDataset3D
...
def train_one_epoch(args, model, train_loader, optimizer, device, losses, is_distributed):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader)):
        *feature_maps, poses, poses_crop, gt = batch
        batch_size = poses.shape[0]
        
        feature_maps = [f.to(device) for f in feature_maps]
        poses, poses_crop, gt = poses.to(device), poses_crop.to(device), gt.to(device)
        ...
        with tmp_context():
            pred = model(feature_maps, poses, poses_crop)
...
def evaluate(args, model, test_loader, datareader, device, rank=0, dist_size=None, master=True):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            *feature_maps, poses, poses_flip, poses_crop, gt = batch
            
            f_len = len(feature_maps) // 2
            
            feature_maps = [f.to(device) for f in feature_maps]
            poses, poses_flip = poses.to(device), poses_flip.to(device),
            poses_crop, gt = poses_crop.to(device), gt.to(device)

            feature_maps_flip = feature_maps[f_len:]
            feature_maps = feature_maps[:f_len] 

            predicted_3d_pos_1 = model(feature_maps, poses, poses_crop)
            poses_crop_flip = flip_data(poses_crop, cropped=True)
            predicted_3d_pos_flip = model(feature_maps_flip, poses_flip, poses_crop_flip)
...
def train(args, opts):
    ...
    train_dataset = MultipleMotionFeatureDataset3D(args, args.subset_list, 'train')
    test_dataset = MultipleMotionFeatureDataset3D(args, args.subset_list, 'test', rank, world_size)
    datareader = MultipleDataReaderH36M(n_frames=args.n_frames, sample_stride=1,
                                        dt_root=args.data_root, data_stride_train=args.n_frames // 3, data_stride_test=args.n_frames)
