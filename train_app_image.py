...
from data.reader.motion_dataset import MultipleMotionImageDataset3D
...
def train_one_epoch(args, model, train_loader, optimizer, device, losses, is_distributed):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader)):
        ...
        images, poses, poses_crop, gt = images.to(device), poses.to(device), poses_crop.to(device), gt.to(device)
...
def evaluate(args, model, test_loader, datareader, device, rank=0, dist_size=None, master=True):
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            images, images_flip, poses, poses_flip, poses_crop, gt = batch
...
def train(args, opts):
    ...
    train_dataset = MultipleMotionImageDataset3D(args, args.subset_list, 'train')
    test_dataset = MultipleMotionImageDataset3D(args, args.subset_list, 'test', rank, world_size)
    ...
    if master:
        ...
        n_params -= count_param_numbers(model.backbone)
        n_params /= 1000000
